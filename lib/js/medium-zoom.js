   ...
   */
    var mediumZoom = function mediumZoom(selector) {
        ...
        try {
          return Array.isArray(selector)
            ...
            : isListOrCollection(selector)
              ? [].concat(toConsumableArray(selector)).filter(isSupported)
              : isNode(selector)
                ? [selector].filter(isSupported)
                : typeof selector === "string"
                  ? []
                      .concat(
                        toConsumableArray(document.querySelectorAll(selector))
                      )
                      .filter(isSupported)
                  : []
                      .concat(
                        toConsumableArray(
                          document.querySelectorAll(
                            SUPPORTED_FORMATS.map(function(attr) {
      ...
      var zoom = function zoom() {
        ...
        if (options.template) {
          var _template = isNode(options.template)
            ? options.template
      ...
      var animateTarget = function animateTarget() {
        ...
        if (options.container) {
          if (options.container instanceof Object) {
            ...
            var zoomContainer = isNode(options.container)
              ? options.container
        ...
        viewportHeight =
          viewportHeight || container.height - options.margin * 2;
        var zoomTarget = target.zoomedHd || target.original;
        var _zoomTarget$naturalWi = zoomTarget.naturalWidth,
          naturalWidth =
            _zoomTarget$naturalWi === undefined
              ? viewportWidth
              : _zoomTarget$naturalWi,
          _zoomTarget$naturalHe = zoomTarget.naturalHeight,
          naturalHeight =
            _zoomTarget$naturalHe === undefined
              ? viewportHeight
              : _zoomTarget$naturalHe;
